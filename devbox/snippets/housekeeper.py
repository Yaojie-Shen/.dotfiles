...
class GPUWorker:
    ...
    def inputs(self):
        if self._inputs is None:
            self._inputs = torch.rand(
                (1, 3, 6, 64, 64), device=torch.device(self.device)
            )
        return self._inputs
    ...
    def net(self):
        if self._net is None:
            self._net = torch.nn.Conv3d(
                3, 3, (4, 16, 16), device=torch.device(self.device)
            )
        return self._net
...
class GPUStatus:
    ...
    def utilization(self):
        ...
        for i in range(60):
            try:
                ...
                utilization = [
                    float(x[:-2]) for x in utilization.split("\n")[1:-1]
                ]  # TODO: Use regx to match
                if len(utilization):
    ...
    def memory(self):
        ...
        for i in range(60):
            try:
                ...
                if self.device_id is not None:
                    cmd += [f"-i={self.device_id}"]
                memory = subprocess.run(args=cmd, stdout=subprocess.PIPE).stdout.decode(
                    "utf-8"
                )
                memory = [
                    float(x[:-4]) for x in memory.split("\n")[1:-1]
                ]  # TODO: Use regx to match
                if len(memory):
        ...
        while True:
            ...
            if cur_utilization is not None and cur_memory is not None:
                with self._access_lock:
                    ...
                    self._utilization_history = self._utilization_history[
                        -short_size + 1 :
                    ]
                    self._memory_history = self._memory_history[-short_size + 1 :]
                    self._utilization_history_long = self._utilization_history_long[
                        -long_size + 1 :
                    ]
                    self._memory_history_long = self._memory_history_long[
                        -long_size + 1 :
                    ]
    ...
    def snapshot(self):
        ...
        with self._access_lock:
            util = round(
                sum(self._utilization_history) / len(self._utilization_history), 1
            )
    ...
    def snapshot_long(self):
        ...
        with self._access_lock:
            util = round(
                sum(self._utilization_history_long)
                / len(self._utilization_history_long),
                1,
            )
            mem = round(
                sum(self._memory_history_long) / len(self._memory_history_long), 1
            )
        return util, mem
    ...
    def is_using(self):
        ...
        return (
            avg_utilization > self._using_utilization_threshold
            and avg_memory > self._using_memory_threshold
        )
...
class GPUStatusController:
    ...
    def __init__(
        ...
        self.status = GPUStatus(
            using_utilization_threshold=utilization_threshold,
...
class CommandController:
    ...
    def __init__(
        ...
        control_file: str = "/tmp/housekeeper_ctl",
    ):
        ...
        try:
            ...
            try:
                ...
                pass
            atexit.register(
                lambda: os.path.exists(self.ctrl_path) and os.remove(self.ctrl_path)
            )
...
def render_ansi(snap: dict) -> str:
    ...
    keep_col = (
        _color("ON", FG_GREEN, True) if s["keeping"] else _color("OFF", FG_RED, True)
    )

    mode_str = str(s["mode"]).upper()
    mode_col = {"AUTO": FG_BLUE, "ENABLED": FG_GREEN, "DISABLED": FG_RED}.get(
        mode_str, FG_GRAY
    )
    dec = str(s["decision"]).lower()
    dec_col = {
        "manual_enable": FG_GREEN,
        "auto_enable": FG_GREEN,
        "enabled": FG_GREEN,
        "manual_disable": FG_RED,
        "auto_disable": FG_RED,
        "disabled": FG_RED,
        "pause": FG_MAGENTA,
    }.get(dec, FG_GRAY)
    ...
    pr = s.get("pause_remaining")
    pt = s.get("pause_total", pr if pr else 0)
    if pr and pt:
...
class Housekeeper:
    def __init__(
        ...
        # controllers
        self.status_ctrl = GPUStatusController(
            utilization_threshold, memory_threshold, using_timeout=10, idle_timeout=10
        )
        self.cmd_ctrl = CommandController(
            control_socket=control_socket, control_file=pause_file
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--control_file",
        default="/tmp/housekeeper_ctl",
        type=str,
        help="control file for pause fallback",
    )
    parser.add_argument(
        "--control_socket",
        default="/tmp/housekeeper_ctl.sock",
        type=str,
        help="unix socket for control",
    )
    parser.add_argument(
        "--ui",
        choices=["line", "ansi", "none"],
        default="none",
        help="UI mode: line|ansi|none (no UI output)",
    )
    args = parser.parse_args()

    if not TORCH_AVAILABLE:
        raise SystemExit(
            "Error: torch is required to run housekeeper (pip install torch)"
        )

    housekeeper = Housekeeper(
        devices=list(range(torch.cuda.device_count())),
        pause_file=args.control_file,
